...
class CreditCardPotManager(OAuthFlowConsumerMixin, Hass):
    ...
    def initialize(self) -> None:
        ...
        self._top_up_listener_registered = False
    ...
    def initialize_entities(self, *, send_notification: bool = True) -> bool:
        ...
        if not self._top_up_listener_registered:
            self.listen_event(
                self.top_up_credit_card_pot,
                "mobile_app_notification_action",
            )
            self._top_up_listener_registered = True
            self.log("Listen event registered for %s", self.credit_card_pot.name)

        self.oauth.clear("cc_pot_top_up")
